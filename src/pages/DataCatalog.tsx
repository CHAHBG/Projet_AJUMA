...
const DataCatalog = () => {
  ...
  const fetchDatasets = async () => {
    ...
    try {
      ...
      if (!error && data) {
        const supabaseDatasets = data.map((d: any) => ({
          id: d.id,
          name: d.name,
        ...
        setDatasets([...mockDatasets, ...supabaseDatasets]);
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (uploadType === "GeoJSON") {
        ...
        const toInsert = csv.data.map((row: any) => ({
          name: row.name || uploadName,
          type: row.type || "bin",
          latitude: parseFloat(row.latitude) || 0,
          longitude: parseFloat(row.longitude) || 0,
          commune_id: row.commune_id || "",
          capacity_kg: parseFloat(row.capacity_kg) || 0,
          waste_type: row.waste_type || "general",
          status: row.status || "active",
        }));
        const { error } = await supabase.from("collection_points").insert(toInsert);
        ...
        const { data, error } = await supabase.storage
          .from("documents")
          .upload(`pdf/${Date.now()}_${uploadFile.name}`, uploadFile, { upsert: true });
        if (error) throw error;
        // Pour référencer le PDF dans une table, ajoute ici l'insertion supabase.from("documents").insert(...)
      }
